...
**/
	L->First = NULL; // ukazatel na prvni prvek
	L->Last = NULL; // ukazatel na posledni prvek
	L->Act = NULL; // ukazatel na aktualni prvek
}
...
**/
	...
	{
		L->Last = tmpPtr; // pokud je seznam prazdnny, tak prvni je zaroven posledni
		L->First = tmpPtr;
		return;
	}
	
	else // pokud neni prazdny
	{
		tmpPtr->rptr = L->First; // prvni prvek na druhy
		L->First->lptr = tmpPtr; // druhy prvek na prvni
		L->First = tmpPtr; // prvni prvek udelam prvnim
	} 
}
...
**/ 	
	...
	{
		tmpPtr->lptr = L->Last; // posledni prvek na predposledni
		L->Last->rptr = tmpPtr; // predposledni na posledni
		L->Last = tmpPtr; // posledni prvek udelam poslednim
	} 
}
...
**/
	...
	if (L->First == NULL)
	{
		return; // nic se nedeje
	}
	
	if (L->Act == L->First)
	{
		L->Act = NULL; // rusim aktivitu
	}
	
	if (L->Last == L->First) //pokud prvni = posledni, tak posledni nulluju
	{
		L->Last = NULL;
	}
...
** volá funkci DLError().
**/
	if (L->Act == NULL) // pokud neni aktivni
	{
		return;
	}
	
	tDLElemPtr tmpPtr = (tDLElemPtr)malloc(sizeof(struct tDLElem)); // alokace
	
	if (tmpPtr == NULL) // malo pameti
	{
		DLError();
		return;
	}

	if (L->Act->rptr == NULL) // pokud vkladam na konec
	{
		L->Last = tmpPtr; // vkladam posledni
	}
	else
	{
		L->Act->rptr->lptr = tmpPtr; // jinak navazu
	}
...
**/
	...
	if (L->Act->lptr == NULL) // pokud vkladam na zacatek
	{
		L->First = tmpPtr; // vkladam posledni
	}
	else
	{
		L->Act->lptr->rptr = tmpPtr; // jinak navazu
	}
