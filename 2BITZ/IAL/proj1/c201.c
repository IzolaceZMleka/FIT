...
***/
	L->Act = NULL;
	tElemPtr tmpPtr = NULL;
	
	while (L->First != NULL) // dokud neni prazdy seznam
	{
		tmpPtr = L->First->ptr;
...
** volá funkci Error().
**/
	tElemPtr tmpPtr = (tElemPtr)malloc(sizeof(struct tElem)); // alokacee
	if(tmpPtr == NULL) // pokud nevyjde, tak error
	{
		Error();
		return;
	}
	
	tmpPtr->data = val; // vlozim data
	tmpPtr->ptr = L->First; // vlozim adresu prvni polozky
	L->First = tmpPtr; // zmenim poradi prvku
}
...
**/
	...
	if(L->First == L->Act)
	{
		L->Act = NULL; // v pripade aktivity se rusi
	}
	
	if(L->First == NULL)
	{
		return; // pokud je prazdny, tak nic nedelam
	}
	
		tmpPtr = L->First; // ukazatel na ruseny prvek
		L->First = L->First->ptr; // prehodim prvek
		free(tmpPtr); // smazu prvek
}	
...
**/
	...
	{
		tmpPtr = L->Act->ptr; // ukazatel na budouci ruseny prvek
		L->Act->ptr = L->Act->ptr->ptr; // prehodim prvek
		free(tmpPtr); // smazu prvek
	}
}
...
**/
	if(L->Act != NULL)
	{
		tElemPtr tmpPtr = (tElemPtr)malloc(sizeof(struct tElem)); // alokace
		
		if(tmpPtr == NULL)
		{
			Error(); // pri neuspechu error
			return;
		}
		else
		{
			tmpPtr->ptr = L->Act->ptr; // adresa prvku za nim
			tmpPtr->data = val; // ulozim data do prvku
			L->Act->ptr =tmpPtr; // prvek po aktivnim je novy
		}
	}
}
