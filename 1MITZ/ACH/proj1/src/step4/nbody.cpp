...
void particles_simulate(t_particles &p)
{
    ...
    float pomx = 0.0f, pomy = 0.0f, pomz = 0.0f;
        ...
        for (i = 0; i < N; i++)
        {
            ...
            for (j = (i+1); j < N; j++){
                ...
                if(r > COLLISION_DISTANCE)
                {
                    ...
                    velocity = ((((G * p1.weight * p2.weight) / (r*r)) / p2.weight) * DT) / r;
                ...
                if (r > 0.0f && r < COLLISION_DISTANCE) {
                    ...
                    pomx += ((2 * p1.weight * p1.vel_x + dw * p2.vel_x) / totalw) - p2.vel_x;
                    pomy += ((2 * p1.weight * p1.vel_y + dw * p2.vel_y) / totalw) - p2.vel_y;
                    pomz += ((2 * p1.weight * p1.vel_z + dw * p2.vel_z) / totalw) - p2.vel_z;
                }
